...
rest.get(Routes.applicationCommands('632956109136855060'))
    .then(data => {
        const promises = [];
        for (const command of data) {
            const deleteUrl = `${Routes.applicationCommands('632956109136855060')}/${command.id}`;
...
rest.get(Routes.applicationGuildCommands('632956109136855060', '632957557375500299'))
    .then(data => {
        const promises = [];
        for (const command of data) {
            const deleteUrl = `${Routes.applicationGuildCommands('632956109136855060', '632957557375500299')}/${command.id}`;
            promises.push(rest.delete(deleteUrl));
        }
        return Promise.all(promises);
    }
...
rest.put(Routes.applicationCommands('632956109136855060'), { body: commands })
    .then(() => console.log("Commandes déployées avec succès"))
    .catch(console.error)

rest.put(Routes.applicationGuildCommands('632956109136855060', '632957557375500299'), { body: devCommands })
